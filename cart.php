...
$connect = mysqli_connect("localhost", "root", "", "user"); //connects to db
...
if(isset($_POST["add_to_cart"])) // what to do if submit is clicked
{
	if(isset($_SESSION["shopping_cart"]))
	{
		...
		else//if item is already present
		{
			...
			echo '<script>window.location="index.php"</script>'; //comes back to the cart 
		}
	}
	...
	if($_GET["action"] == "delete")  //one of the the things passes (checks if action is delete)
	{
		foreach($_SESSION["shopping_cart"] as $keys => $values) //to access each value of shoppingcart
		{
			if($values["item_id"] == $_GET["id"])  //second variable passed (which id to delete)
			{
				...
				echo '<script>window.location="index.php"</script>';  //redirection after removal
			}
		}
	}
}
...
<html>
	...
	<body>
		...
		<div class="container">
			...
			<br /><br />
			<?php
				$query = "SELECT * FROM cart ORDER BY id ASC"; //query
			...
			<div class="col-md-4"> <!class of bootstrap library>
				<form method="post" action="index.php?action=add&id=<?php echo $row["id"]; ?>">
					<div style="border:1px solid #333; background-color:#f1f1f1; border-radius:5px; padding:16px;" align="center">
						<img src="pic1.jpeg" class="img-responsive" /><br /> <!Displays image from db>

						<h4 class="text-info"><?php echo $row["item"]; ?></h4> <!displays name of product>

						<h4 class="text-danger">$ <?php echo $row["price"]; ?></h4><!displays price>

						<input type="text" name="quantity" value="1" class="form-control" /> <!user can enter quantity>

						<input type="hidden" name="hidden_name" value="<?php echo $row["item"]; ?>" /> <!saves product name for future use>

						<input type="hidden" name="hidden_price" value="<?php echo $row["price"]; ?>" /> <!saves price for future use >
				...
				<form method="post" action="index.php?action=add&id=<?php echo $row["id"]; ?>">
					<div style="border:1px solid #333; background-color:#f1f1f1; border-radius:5px; padding:16px;" align="center">
						<img src="https://cdn-ami-drupal.heartyhosting.com/sites/muscleandfitness.com/files/images/bread.jpg" class="img-responsive" /><br /> <!Displays image from db>
